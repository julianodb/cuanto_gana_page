...
export default {
  ...
  watch: {
    async searchedName() {
      ...
      } while(searchTerm.length < maxChars && index.children.length > 0 && index.children.includes(slug.slice(0,searchTerm.length + 1)))
      if(searchTerm.length < maxChars && index.children.length > 0) return this.persons = []
      let names = []
